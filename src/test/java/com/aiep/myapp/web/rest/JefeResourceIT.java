package com.aiep.myapp.web.rest;

import static com.aiep.myapp.domain.JefeAsserts.*;
import static com.aiep.myapp.web.rest.TestUtil.createUpdateProxyForBean;
...
import com.aiep.myapp.IntegrationTest;
import com.aiep.myapp.domain.Jefe;
import com.aiep.myapp.repository.JefeRepository;
...
class JefeResourceIT {
    ...
    @Mock
    private JefeRepository jefeRepositoryMock;
    ...
    void getAllJefesWithEagerRelationshipsIsEnabled() throws Exception {
        when(jefeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restJefeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(jefeRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllJefesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(jefeRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restJefeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(jefeRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    void partialUpdateJefeWithPatch() throws Exception {
        ...
        partialUpdatedJefe.telefonoJefe(UPDATED_TELEFONO_JEFE);
