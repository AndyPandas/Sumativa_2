...
/**
 * REST controller for managing {@link com.aiep.myapp.domain.Jefe}.
 */
@RestController
...
public class JefeResource {
     ...
     */
    @GetMapping("")
    public List<Jefe> getAllJefes(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        LOG.debug("REST request to get all Jefes");
        if (eagerload) {
            return jefeRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Jefe> getJefe(@PathVariable("id") Long id) {
        ...
        Optional<Jefe> jefe = jefeRepository.findOneWithEagerRelationships(id);
